...
complex*16,allocatable :: rho(:,:,:),rho1(:),rho2(:),rho3(:)
...
real*8 expec_sqr_q00(g,g),expec_sqr_q01(g,g),expec_sqr_q10(g,g),expec_sqr_q02(g,g),expec_sqr_q20(g,g)
real*8 expec00(g,g),expec01(g,g),expec10(g,g),expec02(g,g),expec20(g,g)
real*8 expec_q00(g,g),expec_q01(g,g),expec_q10(g,g),expec_q20(g,g),expec_q02(g,g)
...
!stop
  ...
  allocate(rho(nquant,nquant,i),rho1(i),rho2(i),rho3(i))
...
subroutine evolve_classical(dt)
  ...
  do i=1,nclass-2
    x(i)=x(i)+v(i)*dt+0.5*acc(i)*dt*dt
  enddo
  do i=nclass-1,nclass
    x(i)=x(i)+c1*dt*v(i)+c2*dt*dt*acc(i)+delta_r(i)
  enddo
  ...
  do i=1,nclass-2
    v(i)=v(i)+0.5*(acc(i)+acc_sav(i))*dt
  enddo
  do i=nclass-1,nclass
    v(i)=c0*v(i)+(c1-c2)*dt*acc_sav(i)+c2*dt*acc(i)+delta_v(i)
  enddo
  call evaluate_variables(1)
  ...
  if(nflag==0) then
    if(iwrite==1) then
      if(mod(n,nstep_write)==1.or.nstep_write==1) then
        ...
        write(15,'(10es15.5)')curr_time*1.d15,delr(1,1,1)*1.d10,delr(2,2,1)*1.d10,delr(3,3,1)*1.d10
        !do i=1,nclass
...
do j=1,nquant/3
rho1(:)=rho1(:)+rho(j,j,:)
rho2(:)=rho2(:)+rho(j+nquant/3,j+nquant/3,:)
rho3(:)=rho3(:)+rho(j+2*nquant/3,j+2*nquant/3,:)
 enddo


  do i=1,nsteps/nstep_avg
    write(100,'(16f15.7)')(i-1)*nstep_avg*dtc*1.d15,real(rho1(i)),real(rho2(i)),real(rho3(i))
  enddo
...
!x33=(nclass-2)
 ...
 w_c=omg_max/10d0
delw=omg_max/real(nclass)
!write(*,*) omg_max,w_c,delw
  do i=1,x11
    !omg(i)=i*delw
    omg(i)=-w_c*log((i-0.5d0)/float(x11))
    !ck(i)=dsqrt(2*spectral(omg(i))*mass(i)*omg(i)*delw/pi)
    ck(i)=omg(i)*dsqrt(2d0*gamma_B*(mass(1)**2)*w_c/(x11*pi))
!write(20,*) omg(i)/(2*pi*clight),spectral(omg(i))/wave_to_J
!write(1,*) omg(i),ck(i)
  enddo
!stop
!omg((1+x11):x22)=omg(1:x11)
!omg((1+x22):x33)=omg(1:x11)
omg(nclass-1)=omg_B
omg(nclass)=omg_B
!ck(1+(nclass-2)/3:2*(nclass-2)/3)=ck(1:(nclass-2)/3)
!ck(1+2*(nclass-2)/3:(nclass-2))=ck(1:(nclass-2)/3)
!  do i=2,nquant
 !   omg((i-1)*nw+1:i*nw)=omg(1:nw)
  !  ck((i-1)*nw+1:i*nw)=ck(1:nw)
  !enddo

g_coup=dsqrt(2.0d0*mass(1)*lambda_B)*omg_B
...
H6=0.5d0*sum(((ck(1:x11))**2)/(mass(1)*omg(1:x11)**2))
H7=sum(x(1:x11)*ck(1:x11))
...
H_diab(i,i)=H_diab(i,i)+(Hamil_site(1,1)+ens0(i)+g_coup*x(nclass-1))!*expec00(i,j)
H_diab(i+nquant/3,i+nquant/3)=H_diab(i+nquant/3,i+nquant/3)+(Hamil_site(2,2)+ens1(i)+g_coup*x(nclass)/sqrt(2d0))!*expec11(i,j)
H_diab(i+2*nquant/3,i+2*nquant/3)=H_diab(i+2*nquant/3,i+2*nquant/3)+(Hamil_site(3,3)+ens2(i)+g_coup*x(nclass)/sqrt(2d0))!*expec22(i,j)
!write(2,*)H_diab(i,i)/wave_to_j
!write(2,*)H_diab(i,i)/wave_to_j

delV_dels(i,i,nclass-1)=delV_dels(i,i,nclass-1)+g_coup!*expec00(i,j)
delV_dels(i+nquant/3,i+nquant/3,nclass)=delV_dels(i+nquant/3,i+nquant/3,nclass)+g_coup/sqrt(2d0)!*expec11(i,j)
delV_dels(i+2*nquant/3,i+2*nquant/3,nclass)=delV_dels(i+2*nquant/3,i+2*nquant/3,nclass)+g_coup/sqrt(2d0)!*expec22(i,j)

do j=1,nquant/3

H_diab(i,j)=H_diab(i,j)-(H7)*expec_q00(i,j)!+(H6)*expec_sqr_q00(i,j)
H_diab(i+nquant/3,j+nquant/3)=H_diab(i+nquant/3,j+nquant/3)-(H7)*expec_q11(i,j)!+(H6)*expec_sqr_q11(i,j)
H_diab(i+2*nquant/3,j+2*nquant/3)=H_diab(i+2*nquant/3,j+2*nquant/3)-(H7)*expec_q22(i,j)!+(H6)*expec_sqr_q22(i,j)


  H_diab(i,j+nquant/3)= H_diab(i,j+nquant/3)+Hamil_site(1,2)*(expec01(i,j))
  H_diab(i,j+2*nquant/3)=H_diab(i,j+2*nquant/3)+Hamil_site(1,3)*(expec02(i,j))
  H_diab(i+nquant/3,j)=H_diab(i+nquant/3,j)+Hamil_site(2,1)*(expec10(i,j))
  H_diab(i+2*nquant/3,j)=H_diab(i+2*nquant/3,j)+Hamil_site(3,1)*(expec20(i,j))
  H_diab(i+nquant/3,j+2*nquant/3)=H_diab(i+nquant/3,j+2*nquant/3)+Hamil_site(2,3)*(expec12(i,j))
  H_diab(i+2*nquant/3,j+nquant/3)=H_diab(i+2*nquant/3,j+nquant/3)+Hamil_site(3,2)*(expec21(i,j))

!write(2,*)H_diab(i,j+nquant/3)/wave_to_j,expec01(i,j)

delV_dels(i,j,1:x11)=delV_dels(i,j,1:x11)-ck(1:x11)*expec_q00(i,j)
delV_dels(i+nquant/3,j+nquant/3,1:x11)=delV_dels(i+nquant/3,j+nquant/3,1:x11)-ck(1:x11)*expec_q11(i,j)
delV_dels(i+2*nquant/3,j+2*nquant/3,1:x11)=delV_dels(i+2*nquant/3,j+2*nquant/3,1:x11)-ck(1:x11)*expec_q22(i,j)
...
!-----------------------------------------------------------------  
subroutine potential_classical(pot_cl,dpotcl_dx)
  implicit none
  real*8,intent(out) :: pot_cl,dpotcl_dx(nclass)
  integer i
   real*8 H1,H2,H3,H4
   real*8 tmp,mw2
...
H1=0.5d0*mass(1)*(omg_B**2)*((x(nclass-1)**2)+(x(nclass)**2))
!H2=0.5d0*sum(mass(1)*((omg(1:x11))**2)*(x(1:x11)-(x(nclass-1)*ck(1:x11)/(mass(1)*(omg(1:x11))**2)))**2)
!H3=0.5d0*sum(mass(1)*((omg(1:x11))**2)*(x((x22+1):x33)-(x(nclass)*ck(1:x11))/(mass(1)*(omg(1:x11))**2))**2)
H4=0.5d0*sum(mass(1)*((omg(1:x11))**2)*(x(1:x11))**2)

pot_cl=H1+H4

dpotcl_dx=mass(1)*(omg**2)*x
!dpotcl_dx(1:x11)=dpotcl_dx(1:x11)-ck(1:x11)*x(nclass-1)
!dpotcl_dx((1+x22):x33)=dpotcl_dx((1+x22):x33)-ck(1:x11)*x(nclass)
!dpotcl_dx(nclass-1)=dpotcl_dx(nclass-1)+sum((ck(1:x11)**2)*x(nclass-1)/(mass(1)*(omg(1:x11))**2))-sum(ck(1:x11)*x(1:x11))
!dpotcl_dx(nclass)=dpotcl_dx(nclass)+sum(((ck(1:x11))**2)*x(nclass)/(mass(1)*(omg(1:x11))**2))-sum(ck(1:x11)*x((1+x22):x33))
...
!write(*,*)
!wave_to_j,0.5d0*hbar*omg_B/wave_to_j,(g_coup**2/(2d0*mass(1)*omg_B**2))/wave_to_j

dx=(x_grid_max-x_grid_min)/float(g-1)
...
do j=1,g
if (i==j) then
sho_hamil_1(i,j)=0.5d0*mass(1)*(omg_B**2)*(x_grid(i)**2)+(g_coup*x_grid(i)/sqrt(2d0))+(((hbar*pi)**2)/(6.d0*mass(1)*(dx**2)))+0.5d0*sum(((ck(1:x11))**2)/(mass(1)*omg(1:x11)**2))*x_grid(i)**2!+x(i)*ck(i)*x_grid(i)
else
...
do j=1,g
if (i==j) then
sho_hamil_2(i,j)=0.5d0*mass(1)*(omg_B**2)*(x_grid(i)**2)-(g_coup*x_grid(i)/sqrt(2d0))+(((hbar*pi)**2)/(6.d0*mass(1)*(dx**2)))+0.5d0*sum(((ck(1:x11))**2)/(mass(1)*omg(1:x11)**2))*x_grid(i)**2!+x(i)*ck(i)*x_grid(i)
else
...
do j=1,g
if (i==j) then
sho_hamil_0(i,j)=0.5d0*mass(1)*(omg_B**2)*(x_grid(i)**2)+((hbar*pi)**2)/(6.d0*mass(1)*(dx**2))+0.5d0*sum(((ck(1:x11))**2)/(mass(1)*omg(1:x11)**2))*x_grid(i)**2!+x(i)*ck(i)*x_grid(i)
else
...
do i=1,nquant/3
do j=1,nquant/3
expec_q00(i,j)=sum(vect0(:,i)*x_grid*vect0(:,j))
expec_q01(i,j)=sum(vect0(:,i)*x_grid*vect1(:,j))
expec_q10(i,j)=sum(vect1(:,i)*x_grid*vect0(:,j))
expec_q02(i,j)=sum(vect0(:,i)*x_grid*vect2(:,j))
expec_q20(i,j)=sum(vect2(:,i)*x_grid*vect0(:,j))
expec_sqr_q00(i,j)=sum(vect0(:,i)*(x_grid*x_grid)*vect0(:,j))
expec_sqr_q01(i,j)=sum(vect0(:,i)*(x_grid*x_grid)*vect1(:,j))
expec_sqr_q10(i,j)=sum(vect1(:,i)*(x_grid*x_grid)*vect0(:,j))
expec_sqr_q02(i,j)=sum(vect0(:,i)*(x_grid*x_grid)*vect2(:,j))
expec_sqr_q20(i,j)=sum(vect2(:,i)*(x_grid*x_grid)*vect0(:,j))
expec00(i,j)=sum(vect0(:,i)*vect0(:,j))
expec01(i,j)=sum(vect0(:,i)*vect1(:,j))
expec10(i,j)=sum(vect1(:,i)*vect0(:,j))
expec02(i,j)=sum(vect0(:,i)*vect2(:,j))
expec20(i,j)=sum(vect2(:,i)*vect0(:,j))
...
expec_sqr_q11(i,j)=sum(vect1(:,i)*(x_grid*x_grid)*vect1(:,j))
expec_sqr_q22(i,j)=sum(vect2(:,i)*(x_grid*x_grid)*vect2(:,j))
expec_sqr_q12(i,j)=sum(vect1(:,i)*(x_grid*x_grid)*vect2(:,j))
expec_sqr_q21(i,j)=sum(vect2(:,i)*(x_grid*x_grid)*vect1(:,j))
